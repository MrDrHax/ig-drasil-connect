...
async def get_average_call_time(token: Annotated[str, Depends(requireToken)])->models.GenericCard:
    ...
    response = await cachedData.get("get_average_call_time")
...
async def get_agent_rating(agent_id: str, token: Annotated[str, Depends(requireToken)]) -> models.GenericCard:
    ...
    response = await cachedData.get("get_agent_rating", agent_id=agent_id)
...
async def get_schedule(token: Annotated[str, Depends(requireToken)], agent_id:str):
    if not userType.isAgent(token):
        raise HTTPException(status_code=401, detail="Unauthorized. You must be a manager to access this resource.")
...
async def agent_last_contact(token: Annotated[str, Depends(requireToken)], agent_id: str):
    if not userType.isAgent(token):
        raise HTTPException(status_code=401, detail="Unauthorized. You must be a manager to access this resource.")
    
    response = await cachedData.get("agent_last_contact", agent_id=agent_id)
    
    return response

@router.get("/usename", tags=["data"])
async def get_usename(token: Annotated[str, Depends(requireToken)], agent_id:str):
    if not userType.isAgent(token):
        raise HTTPException(status_code=401, detail="Unauthorized. You must be a manager to access this resource.")
...
async def get_alert_agent_NonResponse(agent_id:str, token: Annotated[str, Depends(requireToken)]):
    ...
    response = await cachedData.get("get_alert_agent_NonResponse", agent_id=agent_id)
